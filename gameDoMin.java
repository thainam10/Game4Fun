...
public class gameDoMin extends JFrame implements ActionListener, KeyListener {
	...
	private Color background_number_cl = Color.pink;
	private Color background_null_cl = Color.lightGray;
	private Color flag_cl = Color.cyan;
	private int m, n;
	int M[] = {8, 15, 21};
	int N[] = {10, 19, 27};
	int Mines[] = {10, 40, 100};
...
//		}
	...
	public void initBom() {
		...
		for (i = 0; i <= m + 1; i++)
			for (j = 0; j <= n + 1; j++)
				values[i][j] = 0;
		while (dem < BOM) {
			// Chọn ra toạ độ chưa phải là mìn.
			do {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand() == repeat_bt.getText()) {
			new gameDoMin("Game - Dò Mìn JAVA", lv.getSelectedIndex());
			this.dispose();
		}
		else if (!die) {
			timer.start();
			...
			if (!flag) {
				if (values[i][j] == -1 ) {
					if (bt[i][j].getBackground() != flag_cl) {
						...
						timer.stop();
						loss();
						JOptionPane.showMessageDialog(null, "Trúng Mìn, bạn đã thua!");
						die = true;
					}
				}
	...
	public static void main(String[] args) {
		new gameDoMin("Game - Dò Mìn JAVA", 0);
	}
}